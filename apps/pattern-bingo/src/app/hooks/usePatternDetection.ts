...
import { PATTERN_MULTIPLIERS, PATTERN_POINTS } from '../domain'
...
const AVAILABLE_PATTERNS: readonly PatternType[] = ['LINE', 'CORNERS', 'FRAME', 'PLUS', 'FULL_HOUSE'] as const
...
export const usePatternDetection = () => {
  ...
  const detectAllPatterns = useCallback(() => {
    ...
    if (lineCoords.length > 0) {
      patterns.push({ type: 'LINE', coords: lineCoords })
    }

    const cornersCoords = detectCorners()
    if (cornersCoords.length > 0) {
      patterns.push({ type: 'CORNERS', coords: cornersCoords })
    }

    const frameCoords = detectFrame()
    if (frameCoords.length > 0) {
      patterns.push({ type: 'FRAME', coords: frameCoords })
    }

    const plusCoords = detectPlus()
    if (plusCoords.length > 0) {
      patterns.push({ type: 'PLUS', coords: plusCoords })
    }

    const fullHouseCoords = detectFullHouse()
    if (fullHouseCoords.length > 0) {
      patterns.push({ type: 'FULL_HOUSE', coords: fullHouseCoords })
    }
      ...
      .filter(
        (p) =>
          !detectedPatterns.some(
            (dp) =>
              dp.type === p.type &&
              dp.coordinates.every(([r, c]) => p.coords.some(([pr, pc]) => pr === r && pc === c)),
          ),
      )
      .map((p) => {
  ...
  const availablePatterns = AVAILABLE_PATTERNS

  const currentMultiplier = getNextMultiplier()

  const patternScores = PATTERN_SCORES

  const totalScore = getTotalScore()

  const multiplier = getNextMultiplier()

  const nextMultiplierProgress = (() => {
