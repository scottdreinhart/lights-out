...
/**
 * Bingo Survival Application - Limited time survival mode
 */
export function App() {
  ...
  const { scoreWin, getLeaderboard, getStats } = useScoring('survival', gameState)
  ...
  if (phase === 'splash') return <SplashScreen onComplete={handleSplashComplete} onHowToPlay={handleHowToPlay} onLetsPlay={handleLetsPlay} title="BINGO SURVIVAL" />
  if (phase === 'help') return <div className="bingo-help-screen"><h2>How to Play Bingo Survival</h2><p>Survive the time limit! Mark patterns before time runs out. The longer you last, the harder it gets!</p><button onClick={handleLetsPlay} className="bingo-action-button">Let's Play</button></div>

  return (
    <VariantProvider variantId="survival">
    <div className="bingo-container">
      <div className="bingo-app-header"><div className="app-header-content"><h1 className="app-title">Bingo Survival</h1><div className="header-controls"><HamburgerMenu onRules={() => setShowRules(true)} onSettings={() => setShowSettings(true)} onAbout={() => setShowAbout(true)} /></div></div></div>
