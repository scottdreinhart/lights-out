...
export const reduceGameState = (state: GameState, action: GameAction): GameState => {
  ...
  if (action === 'tick') {
    const next = {
      ...
      status: 'Side Scroller pressure rising',
    }
  ...
  if (action === 'secondary') {
    const next = {
      ...state,
      focus: clamp(state.focus + 7, 0, 100),
      intensity: clamp(state.intensity + -5, 0, 100),
      score: state.score + 2,
      status: 'Stabilize recovered control',
    }
    return withGameOverCheck(next)
  }
