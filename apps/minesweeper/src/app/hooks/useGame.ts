...
const countFlags = (board: Board): number =>
  board.flat().filter((cell) => cell.state === 'flagged').length
...
const isBoardSolved = (board: Board): boolean =>
  board
    .flat()
    .every((cell) =>
      cell.mine ? cell.state === 'flagged' || cell.state === 'revealed' : cell.state === 'revealed',
    )
  ...
  const reveal = useCallback((row: number, col: number) => {
    setGame((current) => {
      ...
      const boardWithMines =
        current.status === 'idle'
          ? placeMinesWithEngine(current.board, current.mines, row, col)
          : current.board
