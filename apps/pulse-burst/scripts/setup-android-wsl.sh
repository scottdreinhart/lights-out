...
if [ "$SKIP_DL" = true ] && [ ! -d "${ANDROID_SDK}/cmdline-tools/latest" ]; then
  echo ""
  echo -e "${YELLOW}==================================================${RESET}"
  echo -e "${YELLOW}Manual SDK Installation${RESET}"
  echo -e "${YELLOW}==================================================${RESET}"
  echo ""
...
echo -e "${YELLOW}Installing SDK components...${RESET}"
...
echo -e "${YELLOW}STEP 4: System Images${RESET}"
echo -e "${YELLOW}Checking available system images...${RESET}"
echo ""
...
echo -e "${YELLOW}STEP 5: Creating Android Virtual Devices${RESET}"
echo ""
