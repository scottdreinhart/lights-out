...
test.describe('MainMenu Responsive Scaling', () => {
  test('renders at all breakpoints without overflow', async ({ page, allBreakpoints }) => {
    ...
    for (const bp of allBreakpoints) {
      ...
      if (box) {
        ...
        console.log(
          `✓ ${bp.label}: Content width ${Math.round(box.width)}px (viewport: ${bp.width}px)`,
        )
      }
    }
  })
  ...
  test('scales button containers properly', async ({ page, allBreakpoints }) => {
    ...
    for (const bp of allBreakpoints) {
      ...
      if (count > 1) {
        ...
        if (box1 && box2) {
          console.log(
            `✓ ${bp.label}: Found ${count} buttons, first at ${Math.round(box1.x)},${Math.round(box1.y)}, second at ${Math.round(box2.x)},${Math.round(box2.y)}`,
          )
        }
      }
    }
  ...
  test('no horizontal scrollbar at any breakpoint', async ({ page, allBreakpoints }) => {
    ...
    for (const bp of allBreakpoints) {
      ...
      console.log(
        `✓ ${bp.label}: No horizontal overflow (scroll: ${scrollWidth}px <= viewport: ${clientWidth}px)`,
      )
    }
  })
