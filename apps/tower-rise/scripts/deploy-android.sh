...
list_connected_devices() {
  echo -e "${BLUE}Connected devices (via adb):${RESET}"
...
list_available_avds() {
  echo -e "${BLUE}Available Android Virtual Devices (AVDs):${RESET}"
...
select_device() {
  ...
  if [ "$input" = "auto" ]; then
    ...
    if [ "$count" -eq 0 ]; then
      echo -e "${YELLOW}No devices connected. Options:${RESET}"
      echo ""
      ...
      echo -e "${YELLOW}Multiple devices detected. Select one:${RESET}"
      list_connected_devices
      echo ""
...
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo -e "${BLUE}Android Deployment Helper (Emulators + Real Devices)${RESET}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo ""

case "${TASK}" in
  launch|start)
    echo -e "${YELLOW}Launching emulator: ${EMULATOR_NAME} (headless)${RESET}"
    emulator -avd "${EMULATOR_NAME}" -no-window &
    sleep 5
    echo -e "${GREEN}✓ Emulator launched${RESET}"
    echo "Wait for boot (30-60 seconds), then run:"
    echo "  bash scripts/deploy-android.sh ${EMULATOR_NAME} install"
    ;;

  console)
    echo -e "${YELLOW}Launching emulator: ${EMULATOR_NAME} (with console)${RESET}"
    emulator -avd "${EMULATOR_NAME}" &
    sleep 5
    echo -e "${GREEN}✓ Emulator launched with UI${RESET}"
    ;;

  build)
    echo -e "${YELLOW}Building Android APK...${RESET}"
    pnpm build
    ...
    if [ -f "${APK_PATH}" ]; then
      echo -e "${GREEN}✓ APK built: ${APK_PATH}${RESET}"
    else
  ...
  install)
    echo -e "${YELLOW}Installing APK to emulator: ${EMULATOR_NAME}${RESET}"
    APK_PATH="android/app/build/outputs/apk/debug/app-debug.apk"
    
    if [ ! -f "${APK_PATH}" ]; then
      echo -e "${YELLOW}APK not found, building...${RESET}"
  ...
  run|deploy)
    echo -e "${YELLOW}Full deployment: build → install → run${RESET}"
    bash "$0" "${EMULATOR_NAME}" build
    bash "$0" "${EMULATOR_NAME}" install
    echo -e "${YELLOW}Launching app...${RESET}"
    adb shell am start -n com.scottreinhart.nim/.MainActivity
    echo -e "${GREEN}✓ App launched${RESET}"
    ;;

  logcat|logs)
    echo -e "${YELLOW}Streaming logcat (Ctrl+C to exit)${RESET}"
  ...
  clear)
    echo -e "${YELLOW}Clearing app data...${RESET}"
    adb shell am force-stop com.scottreinhart.nim
    adb shell pm clear com.scottreinhart.nim
    echo -e "${GREEN}✓ App data cleared${RESET}"
    ;;

  kill)
    echo -e "${YELLOW}Stopping emulator...${RESET}"
    adb emu kill
    echo -e "${GREEN}✓ Emulator stopped${RESET}"
    ;;
