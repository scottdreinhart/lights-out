 ...
 */
export function getBasicStrategyAction(context) {
    const { playerHand, dealerUpCard, canDouble, canSplit, canSurrender, isSoft, isPair, handValue } = context;
 ...
 */
function getHardTotalAction(handValue, dealerUpCard) {
 ...
 */
function getSoftValue(hand) {
    const values = hand.map((card) => card.rank === 'A' ? 11 : card.rank === '10' ? 10 : parseInt(card.rank));
 ...
 */
export function getStrategyRecommendation(context) {
    const action = getBasicStrategyAction(context);
    // Determine confidence level
    let confidence = 'always';
    let reasoning = '';
    switch (action) {
        case 'stand':
            confidence = 'always';
 ...
 */
function calculateExpectedValue(context, action) {
    ...
    const _dealerValue = dealerUpCard.rank === 'A' ? 11 : dealerUpCard.rank === '10' ? 10 : parseInt(dealerUpCard.rank);
 ...
 */
export function getCountingState(runningCount, decksRemaining) {
 ...
 */
export function getBasicStrategyRecommendation(playerHard, playerSoft, dealerUpCard, playerAction) {
    const context = {
 ...
 */
export function getSplitStrategyRecommendation(cardRank, dealerUpCard, playerAction) {
    const pairKey = `${cardRank},${cardRank}`;
    const dealerRank = dealerUpCard.rank === '10' ? '10' : dealerUpCard.rank;
    const recommendedAction = PAIRS_STRATEGY_TABLE[pairKey]?.[dealerRank] ?? 'stand';
    const isCorrect = (recommendedAction === 'split' && playerAction === 'split') ||
 ...
 */
export function getStrategyAccuracyFeedback(isCorrect, correctStreak) {
    if (!isCorrect) {
 ...
 */
export function createCardCountingState(deckCount) {
    return {
        runningCount: 0,
        trueCount: 0,
 ...
 */
export function calculatePenetration(cardsDealt, totalCards) {
    return totalCards > 0 ? (cardsDealt / totalCards) * 100 : 0;
}
/**
 * Evaluate counting accuracy
 */
export function evaluateCountingAccuracy(playerCount, actualCount) {
 ...
 */
export function getBetSizingStrategy(trueCount, minBet, maxBet) {
    let multiplier = 1;
    if (trueCount >= 5) {
 ...
 */
export function getCountingDifficulty(_trueCount, accuracy) {
    if (accuracy < 50) {
        return 'Very Hard';
    }
    if (accuracy < 70) {
 ...
 */
export function getCountingStrategyAdvice(trueCount) {
    if (trueCount >= 4) {
 ...
 */
export function updateCountingState(state, card, cardsDealt, totalCards) {
    let runningCount = state.runningCount;
    // Hi-Lo counting system
    const cardValue = card.rank === '10' || card.rank === 'J' || card.rank === 'Q' || card.rank === 'K'
        ? '10'
        : card.rank;
