...
export function calculateHandValue(hand) {
    ...
    for (const card of cards) {
        ...
        if (rank === 'A') {
            ...
            // Jokers are not part of blackjack scoring.
            value += 0;
        }
        else {
 ...
 */
export function getHandValues(hand) {
    const cards = Array.isArray(hand) ? hand : hand.cards;
    let hardValue = 0;
    let aces = 0;
    for (const card of cards) {
 ...
 */
export function getBestHandValue(hand) {
 ...
 */
export function isTwentyOne(hand) {
    return calculateHandValue(hand) === 21;
}
// ┌─────────────────────────────────────────────────────────┐
// │ VALID ACTIONS                                           │
...
export function canSplit(hand) {
    ...
    return (cards[0].rank === cards[1].rank ||
        (tenValueRanks.has(cards[0].rank) && tenValueRanks.has(cards[1].rank)));
}
// ┌─────────────────────────────────────────────────────────┐
// │ SETTLEMENT LOGIC                                        │
// └─────────────────────────────────────────────────────────┘
 ...
 */
export function calculatePayout(bet, outcome, isBlackjack) {
    if (outcome === 'push') {
        return 0;
    }
    if (outcome === 'loss') {
 ...
 */
export function createGameState(playerBalance = 1000) {
    return {
        id: `game-${Date.now()}`,
        phase: 'betting',
        players: [
 ...
 */
export function dealInitialHands(gameState, playerCards, dealerCards) {
 ...
 */
export function processPlayerAction(gameState, action) {
    const player = gameState.players[0];
    if (!player) {
        return gameState;
    }
