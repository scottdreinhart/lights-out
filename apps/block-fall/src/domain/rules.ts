...
export const reduceGameState = (state: GameState, action: GameAction): GameState => {
  ...
  if (action === 'tick') {
    const next = {
      ...
      status: 'Falling Block Puzzle pressure rising',
    }
  ...
  if (action === 'secondary') {
    const next = {
      ...
      intensity: clamp(state.intensity + -5, 0, 100),
      score: state.score + 2,
      status: 'Settle Board recovered control',
    }
    return withGameOverCheck(next)
  }
  ...
  const next = {
    ...state,
    score: state.score + 17,
    progress: state.progress + 8,
    intensity: clamp(state.intensity + 9, 0, 100),
    focus: clamp(state.focus - 6, 0, 100),
    status: 'Hard Drop high-risk burst',
  }
  return withGameOverCheck(next)
}
