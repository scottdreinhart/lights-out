...
export function generatePuzzleBatch(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (puzzle) {
      ...
      // testUniquenessTime would be tracked separately if implementing detailed metrics
    }
  }

  const stats: GeneratorStats = {
    puzzlesGenerated: puzzles.length,
    testSolvabilityTime: puzzles.length > 0 ? testSolvabilityTime / puzzles.length : 0,
    testUniquenessTime: puzzles.length > 0 ? testUniquenessTime / puzzles.length : 0,
