...
export abstract class BaseConstraint implements Constraint {
   ...
   */
  abstract isSatisfied(assignment: Map<string, number | string | boolean>): boolean
...
export class CustomConstraint extends BaseConstraint {
  constructor(
    name: string,
    scope: string[],
    private predicate: (assignment: Map<string, number | string | boolean>) => boolean,
  ) {
    super(name, scope)
  }
...
export const ConstraintFactory = {
   ...
   */
  domain(varId: string, allowedValues: Set<number | string | boolean>): DomainConstraint {
