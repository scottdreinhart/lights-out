...
export function addConstraint(csp: CSP, constraint: Constraint): void {
  ...
  for (const scopeVarId of constraint.scope) {
    if (!varIds.has(scopeVarId)) {
      throw new Error(`Constraint scope includes undefined variable: ${scopeVarId}`)
    }
  }
 ...
 */
export function reduceToValue<T = number | string | boolean>(domain: Domain, value: T): boolean {
 ...
 */
export function removeFromDomain<T = number | string | boolean>(domain: Domain, value: T): boolean {
  if (!domain.possible.has(value)) {
    return false
  }
 ...
 */
export function getAssignedValue<T = number | string | boolean>(domain: Domain): T | undefined {
 ...
 */
export function buildAssignmentFromDomains(csp: CSP): Map<string, number | string | boolean> {
 ...
 */
export function getConstraintsForVariable(csp: CSP, variableId: string): Constraint[] {
...
export function getCSPStats(csp: CSP): CSPStats {
  ...
  return {
    ...
    unaryConstraintsCount: csp.constraints.filter((c) => c.scope.length === 1).length,
    binaryConstraintsCount: csp.constraints.filter((c) => c.scope.length === 2).length,
    higherOrderConstraintsCount: csp.constraints.filter((c) => c.scope.length > 2).length,
 ...
 */
export function generateEdgeId(row1: number, col1: number, row2: number, col2: number): string {
