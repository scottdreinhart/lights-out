...
export class BacktrackingSolver implements Solver {
  ...
  solve(csp: CSP, timeout?: number): SolveResult {
    ...
    if (this.verbose) {
      console.log(`[${this.name}] Solved: ${solved}, Iterations: ${iterations}, Time: ${timeMs}ms`)
    }
 ...
 */
function saveDomainState(csp: CSP): Map<string, Set<number | string | boolean>> {
 ...
 */
function restoreDomainState(csp: CSP, saved: Map<string, Set<number | string | boolean>>): void {
  for (const domain of csp.domains) {
