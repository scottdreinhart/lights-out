...
| `apps/lights-out/assembly/tsconfig.json` | AssemblyScript compiler config (extends `assemblyscript/std/assembly.json`) |
| `apps/lights-out/scripts/build-wasm.js` | Node build script — compiles AS → WASM → base64 → `src/wasm/ai-wasm.ts` |
...
apps/lights-out/assembly/index.ts → (pnpm wasm:build) → build/ai.wasm → base64 → src/wasm/ai-wasm.ts → src/workers/ai.worker.ts → UI organism (postMessage / onmessage)
...
Both scripts run `apps/lights-out/scripts/build-wasm.js` which invokes the `asc` (AssemblyScript compiler) CLI.

---

## Language Boundaries

- **AssemblyScript** is the only language for WASM source code (`apps/lights-out/assembly/`).
- **JavaScript** is used for the build script (`apps/lights-out/scripts/build-wasm.js`).
...
Do not create alternative WASM build pipelines outside the existing `apps/lights-out/scripts/build-wasm.js` path.
...
The single build path is `pnpm wasm:build` → `apps/lights-out/scripts/build-wasm.js`.
