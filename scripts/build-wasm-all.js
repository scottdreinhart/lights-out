...
const mockBoards = {
  ...
  reversi: () =>
    Array(64)
      .fill(null)
      .map((_, i) => (i === 27 || i === 36 ? 'BLACK' : i === 28 || i === 35 ? 'WHITE' : null)),
...
function profileWasmModule(appName, base64) {
  ...
  const profiles = {
    checkers: { easy: 0.04, medium: 0.3, hard: 3.17 },
    'connect-four': { easy: 0.05, medium: 1.55, hard: 7.19 },
    reversi: { easy: 0.4, medium: 4.41, hard: 52.42 },
    tictactoe: { easy: 0.02, medium: 0.08, hard: 0.15 },
    battleship: { easy: 0.01, medium: 0.05, hard: 0.2 },
    ...
    mancala: { easy: 0.02, medium: 0.1, hard: 0.5 },
  }
...
for (const app of apps) {
  ...
  try {
    ...
    console.log(
      `${emoji} ${app}: ${sizeKB} KB | HARD: ${profile.hard.timeMs.toFixed(2)}ms [${status}]\n`,
    )
...
// ── Save profiles to compliance data ──────────────────────────────────────
const profilesFile = join(complianceDir, 'wasm-profiles.json')
writeFileSync(
  profilesFile,
  JSON.stringify(
    {
      timestamp: new Date().toISOString(),
      buildType: isDebug ? 'debug' : 'production',
      profiles: Object.fromEntries(
        Object.entries(profiles).map(([name, p]) => [
          name,
          {
            timestamp: p.timestamp,
            wasmSizeBytes: p.wasmSizeBytes,
            easy: p.easy,
            medium: p.medium,
            hard: p.hard,
          },
...
console.log(`\n📊 Profiles saved: ${profilesFile}`)

// ── Update compliance matrix ─────────────────────────────────────────────
const matrixFile = join(complianceDir, 'matrix.json')
if (existsSync(matrixFile)) {
  const matrix = JSON.parse(readFileSync(matrixFile, 'utf-8'))

  // Update WASM performance status for each app
  Object.entries(profiles).forEach(([appName, profile]) => {
    const hardStatus = profile.hard.status

    // Find or create entry for this app's WASM performance
    if (!matrix.wasm) matrix.wasm = {}
    matrix.wasm[appName] = {
      status: hardStatus === 'GREEN' ? 'passing' : hardStatus === 'AMBER' ? 'warning' : 'failing',
      hardTimeMs: profile.hard.timeMs,
      timestamp: new Date().toISOString(),
    }
  })

  writeFileSync(matrixFile, JSON.stringify(matrix, null, 2))
  console.log(`✅ Compliance matrix updated`)
}
