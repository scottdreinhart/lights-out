...
- `electron/main.js` — App lifecycle, window management, IPC handlers
- `electron/preload.js` — Secure context bridge API
...
- **Signing**: For production, add code signing certificates in `electron/main.js`
...
- **Splash Screen**: Branded launch screen (configure in capacitor.config.ts)
...
| Issue | Solution |
|-------|----------|
| App won't start | Check electron/main.js loads dist/index.html correctly |
