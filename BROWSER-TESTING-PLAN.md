...
### Step 3: Mark Verified Apps with `.browser-verified`

After testing an app and confirming it works, mark it:

```bash
# Mark checkers as verified (creates .browser-verified file)
node scripts/mark-app-verified.mjs checkers

# Output:
# ✅ Marked checkers as verified
# 📝 Created: apps/checkers/.browser-verified
# 📊 Updated: 1/38 apps verified
```

**What this does:**
- Creates `apps/checkers/.browser-verified` with metadata
- Updates `compliance/verified-apps.json` with the verified app
- Removes it from pending list
- Tracks verification timestamp and status

**Later, rebuild the verified list:**

```bash
# Scans all apps for .browser-verified markers
node scripts/scan-verified-apps.mjs

# Output shows:
# ✅ Verified: checkers, connect-four, ...
# ⏳ Pending: battleship, bunco, ...
```

### Step 4: Update Tracking Document

After testing, also update the manual tracking:
...
- ✅ PASS - Loads correctly, displays properly, marked verified
...
## 🏷️ Verified App Marker System

**These scripts minimize churn and protect verified apps from unnecessary changes:**

```bash
# After testing checkers and confirming it works:
node scripts/mark-app-verified.mjs checkers

# This creates: apps/checkers/.browser-verified
# And updates: compliance/verified-apps.json
```

**View all verified apps:**

```bash
cat compliance/verified-apps.json

# Shows:
# - verified-count: How many apps are verified
# - verified-apps: List of tested apps
# - pending-apps: List of apps still to test
```

**Rescan all apps for markers:**

```bash
# After marking several apps, rebuild the aggregator:
node scripts/scan-verified-apps.mjs

# Output shows detailed status of all 38 apps
```

**The `.browser-verified` file contains:**

```json
{
  "verified-timestamp": "2026-04-02T01:45:00Z",
  ...
  "notes": "Game loads, renders correctly, no errors"
}
```

**Why this matters:**
- ✅ Protects verified apps from accidental refactors
- ✅ Provides clear audit trail (timestamp + browser)
- ✅ Enables phased optimization (verified first, then fix pending)
- ✅ Minimizes churn while decomposition happens
- ✅ 100% git-friendly (small text files, no code changes)
...
**Once it loads, mark it verified:**
