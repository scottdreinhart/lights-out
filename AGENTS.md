...
- **JavaScript**: Build script (`apps/lights-out/scripts/build-wasm.js`)
- **TypeScript**: Runtime loader (`src/wasm/ai-wasm.ts`), worker (`src/workers/ai.worker.ts`)

**Anti-orphan-script policy**: Single build path only (`pnpm wasm:build` → `apps/lights-out/scripts/build-wasm.js`). Do not create alternative build pipelines. Never edit `src/wasm/ai-wasm.ts` manually.
...
pnpm -C apps/lights-out lint  # Should succeed with root config
