...
- Use Bash (WSL on Windows, native on macOS/Linux) for all WASM builds: `pnpm wasm:build`, `pnpm wasm:build:debug`.
- Never use PowerShell for WASM builds.
- See [AGENTS.md § 5](../../AGENTS.md#5-cross-platform-shell-governance-mandatory) (Cross-platform Shell Governance) for complete shell rules.
...
apps/<game-app>/assembly/index.ts → (pnpm wasm:build) → build/ai.wasm → base64 → src/wasm/ai-wasm.ts → src/workers/ai.worker.ts → UI organism (postMessage / onmessage)
...
- **AssemblyScript** is the only language for WASM source code (`apps/<game-app>/assembly/`).
- **JavaScript** is used for the build script (`apps/<game-app>/scripts/build-wasm.js`).
