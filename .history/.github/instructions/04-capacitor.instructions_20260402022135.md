...
**Bash (POSIX) is the mandatory default shell for all Capacitor development.** macOS is opt-in only.

- **Default**: Use Bash (WSL on Windows, native on macOS/Linux) for all Capacitor setup, initialization, and Android work.
- **macOS Opt-In**: `pnpm cap:init:ios`, `pnpm cap:open:ios`, `pnpm cap:run:ios` require macOS hardware and Xcode, only when explicitly needed.
- **Never**: Assume macOS availability by default; always default to Bash and Android-first for routine Capacitor work.
- **Critical**: iOS builds require Apple hardware. Never suggest iOS Capacitor commands unless the user has confirmed macOS availability.
