...
import { cloneCSP, setClue, buildAssignmentFromDomains, Difficulty } from '../csp'
 ...
 * @param csp CSP instance
 * @param solveIterations Number of iterations solver needed
 * @returns Difficulty score 0-100
 */
function estimateDifficulty(csp: CSP, solveIterations?: number): number {
  ...
  const estimatedMinClues = Math.ceil(variableCount * 0.3)

  // Estimated solve time based on iterations (1ms per 1000 iterations)
  const estimatedSolveMs = (solveIterations ?? 10000) / 1000
