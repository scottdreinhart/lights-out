...
import { isWorkerRequest, generateRequestId } from './messages'

// ============================================================================
// WORKER STATE
// ============================================================================

const WORKER_ID = `worker_${Math.random().toString(36).substr(2, 9)}`
const solver = new BacktrackingSolver({ timeout: 1000 })
const requestMap = new Map<string, any>()  // Track pending requests
  ...
  try {
    switch (msg.type) {
      ...
      case 'PROPAGATE_REQUEST':
        ...
        break
      default:
        handleError(`Unknown request type: ${(msg as any).type}`, msg.requestId)
    }
  } catch (error) {
    handleError(
