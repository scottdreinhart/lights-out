...
### Pattern 1: localStorage-Backed Leaderboard with Podium (NEW)

**Use when**: You want persistent leaderboards with automatic ranking.

```tsx
// apps/your-game/src/ui/organisms/ResultsScreen.tsx
import { useKothData, KothPodium, KothLeaderboard } from '@games/ui-koth-system'

export const ResultsScreen = ({ finalScore, gameDifficulty, gameDuration }) => {
  const { entries, addEntry, getPlayerRank } = useKothData({
    gameName: 'your-game',
  })

  const handleDone = () => {
    // Save result to persistent leaderboard
    addEntry({
      username: 'Player1', // Get from your player state
      score: finalScore,
  ...
  return (
    <div className="results">
      ...
      <KothPodium first={topThree[0]} second={topThree[1]} third={topThree[2]} />
      <KothLeaderboard entries={allEntries} />
...
### Pattern 2: Direct Results Screen

**Use when**: Your game has a traditional full-screen results display.
...
### Pattern 3: Custom Leaderboard with Enhanced Rows (NEW)

**Use when**: You want full control over the leaderboard display with metadata.

```tsx
// apps/your-game/src/ui/organisms/LeaderboardView.tsx
import { useKothData, KothEntryRow } from '@games/ui-koth-system'

export const LeaderboardView = () => {
  const { entries, getPlayerRank } = useKothData({ gameName: 'your-game' })
  const currentUsername = 'Player1'
  const currentPlayerRank = getPlayerRank(currentUsername)
  ...
  return (
    <div className="leaderboard">
      ...
      {currentPlayerRank && <p>Your rank: #{currentPlayerRank}</p>}
    </div>
  )
}
```

### Pattern 4: Stats Service Integration
