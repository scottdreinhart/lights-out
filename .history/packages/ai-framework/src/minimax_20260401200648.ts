...
import type { GameAI, MinimaxOptions, MinimaxResult } from './types'
...
export function minimax<Board, Move, Player>(
  ...
  const orderedMoves =
    moveOrdering && options.moveOrder
      ? [...moves].sort(
          (a, b) => (options.moveOrder!(b, board) ?? 0) - (options.moveOrder!(a, board) ?? 0),
        )
      : moves
...
/**
 * Safe wrapper around minimax with error recovery and fallback strategies
 *
 * This function provides robust error handling and graceful degradation:
 * - Catches minimax errors
 * - Falls back to simple move selection if minimax fails
 * - Retries with reduced depth on timeout
 * - Never throws; always returns a playable move
 *
 * @param board The current board state
 * @param player The player to find best move for
 * @param depth How deep to search the game tree
 * @param game The GameAI implementation for this game
 * @param options Minimax configuration
 * @param recovery Recovery strategy for error handling
 * @returns Safe AI result with the best move and fallback information
 *
 * @example
 * const game = new CheckersAI()
 * const result = safeMinimax(board, 'red', 4, game, {}, { useRandomMove: true })
 * console.log(`Move: ${result.move}, IsFallback: ${result.isFallback}`)
 */
export function safeMinimax<Board, Move, Player>(
  board: Board,
  player: Player,
  depth: number,
  game: GameAI<Board, Move, Player>,
  options = {},
  recovery = {},
): { move: Move; score: number; isFallback: boolean; error?: { type: string; details: string } } {
  const {
    useRandomMove = true,
    useFirstLegalMove = true,
    selectFallbackMove,
    retryWithReducedDepth = true,
    maxRetries = 2,
    logErrors = false,
  } = recovery as any
  ...
  while (attempts <= maxRetries) {
    try {
      const result = minimax(board, player, depth, game, options)
      return {
      ...
      lastError = err
      if (logErrors) {
        console.warn(`Minimax attempt ${attempts + 1} failed:`, err)
      }

      attempts++

      // Retry with reduced depth
      if (retryWithReducedDepth && depth > 1 && attempts <= maxRetries) {
        depth = Math.max(1, depth - 1)
        continue
      }

      // Give up on minimax, use fallback
      break
    }
  }
  ...
  if (logErrors && lastError) {
    console.warn('Minimax failed, using fallback:', lastError)
  }

  const legalMoves = game.getLegalMoves(board, player)
  if (legalMoves.length === 0) {
    throw new Error('No legal moves available (game is in invalid state)')
  }

  let fallbackMove: Move

  if (selectFallbackMove) {
    fallbackMove = selectFallbackMove(legalMoves)
  } else if (useFirstLegalMove) {
    fallbackMove = legalMoves[0]!
  } else if (useRandomMove) {
    fallbackMove = legalMoves[Math.floor(Math.random() * legalMoves.length)]!
  } else {
    fallbackMove = legalMoves[0]!
  }
  ...
  return {
    ...
    isFallback: true,
    error: lastError
      ? { type: 'MINIMAX_FALLBACK', details: String(lastError) }
      : undefined,
  }
}
