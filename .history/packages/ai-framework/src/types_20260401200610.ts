...
/**
 * Error types for AI framework operations
 */
export type AIError = 'NO_LEGAL_MOVES' | 'INVALID_DEPTH' | 'NO_BEST_MOVE' | 'UNSTABLE_EVALUATION' | 'TIMEOUT' | 'UNKNOWN'

/**
 * Result type that either contains a value or an error
 */
export type Result<T> = { ok: true; value: T } | { ok: false; error: AIError; details: string }

/**
 * Recovery strategy when minimax fails
 */
export interface RecoveryStrategy<Move = unknown> {
  /** Use a random legal move as fallback */
  useRandomMove?: boolean
  
  /** Use the first legal move as fallback */
  useFirstLegalMove?: boolean
  
  /** Custom fallback move selector */
  selectFallbackMove?: (moves: Move[]) => Move
  
  /** Retry with reduced depth on failure */
  retryWithReducedDepth?: boolean
...
/**
 * Safe AI decision result with error recovery
 */
export interface SafeAIResult<Move = unknown> {
  move: Move
  score: number
  isFallback: boolean
  error?: { type: AIError; details: string }
}
