...
const describeSquare = (
  ...
  if (!isPlayableSquare(position.row, position.col)) {
    return `Light square ${position.row + 1}, ${position.col + 1}`
  }
  ...
  if (!piece) {
    const target = selected
      ...
      : false
    return `Dark square ${position.row + 1}, ${position.col + 1}${target ? ', legal destination' : ''}`
  }

  const owner = piece.player === 'red' ? 'red checker' : 'black checker'
  const rank = piece.isKing ? ' king' : ''
  const action = selectable ? ', movable' : ''
  return `${owner}${rank} on ${position.row + 1}, ${position.col + 1}${action}${selectedLabel}`
}
