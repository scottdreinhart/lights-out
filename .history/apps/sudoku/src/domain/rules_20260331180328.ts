import type { Board, Cell, Digit, GameState, Difficulty, Move } from './types'
...
export const isValidMove = (state: GameState, row: number, col: number, value: Cell): boolean => {
  ...
  // Can only edit empty cells (cells that were empty at start)
  if (state.solution.grid[row][col] === EMPTY_CELL && currentValue === EMPTY_CELL) {
    return true
  }
...
export const getValidMoves = (state: GameState, row: number, col: number): Digit[] => {
  if (state.solution.grid[row][col] !== EMPTY_CELL) {
    return [] // Can't edit pre-filled cells
  }
