...
import type { CellValue, Difficulty, MiniSudokuState } from '../domain'
...
export function useMiniSudoku(options: UseMiniSudokuOptions = {}): UseMiniSudokuReturn {
  ...
  const isComplete = useCallback(() => Queries.handleIsComplete(state), [state])

  const isSolved = useCallback(() => Queries.handleIsSolved(state), [state])
  ...
  const getGameStats = useCallback(() => Queries.handleGetGameStats(state), [state])

  const getNextHint = useCallback(() => Queries.handleGetNextHint(state), [state])
  ...
  const getConflictingCells = useCallback(
    (cellId: string, value: CellValue) => Queries.handleGetConflictingCells(state, cellId, value),
    [state],
  )
