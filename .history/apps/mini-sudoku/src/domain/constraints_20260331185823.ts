...
import { BOARD_SIZE, BOX_SIZE, parseCellId, getBoxIndex } from './constants'
...
export class BoxUniquenessConstraint extends UniquenessConstraint {
  constructor(boxIndex: number) {
    const cellIds = []
    const startRow = Math.floor(boxIndex / BoxSize) * BOX_SIZE
 ...
 */
export function getConstraintsForCell(cellId: string): MiniSudookConstraint[] {
  const constraints: MiniSudokuConstraint[] = []
