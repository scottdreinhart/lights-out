...
export function estimateDifficulty(board: Board): DifficultyMetrics {
  ...
  const averageC andidates = emptyCount > 0 ? totalCandidates / emptyCount : 0
  const constraintDensity = 1 - averageC andidates / 4 // Higher = more constrained
...
export function estimateSolveTime(board: Board): number {
  ...
  baseTime *= 1 + (4 - metrics.averageC andidates) / 4
