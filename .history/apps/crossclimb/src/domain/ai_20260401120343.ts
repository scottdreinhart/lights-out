...
const heuristic = (nodeId: NodeId, graph: Graph): number => {
  ...
  // Basic distance
  const distance = Math.abs(node.position.x - endNode.position.x) +
                  Math.abs(node.position.y - endNode.position.y)

  // Penalty for unvisited checkpoints
  const unvisitedCheckpoints = graph.checkpoints.filter(cp =>
    !graph.nodes.get(cp)?.visited
  ).length
...
export const getHintMove = (currentPath: NodeId[], graph: Graph): NodeId | null => {
  ...
  for (const neighborId of currentNode.connections) {
    ...
    const distance = Math.abs(neighbor.position.x - graph.nodes.get(graph.endNode)!.position.x) +
