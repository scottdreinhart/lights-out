...
- ✅ Real-time regression alerts (email, dashboard, CI/CD)
- ✅ Per-app performance budgets with enforcement
...
### Sprint 1: Baseline & Regression Detection (Weeks 1–2)

**Week 1: Setup**
- [ ] Create `compliance/wasm-profiles-baseline.json` from current production state
- [ ] Create `compliance/baselines/` directory for historical tracking
- [ ] Write documentation: "How to Create and Update Baselines"
- [ ] Add `compliance/regression-alerts.json` template
- [ ] Update `package.json` to include baseline management scripts

**Task Duration**: 4–6 hours

**Deliverables:**
- Baseline snapshot file (approved by you)
- Directory structure for historical baselines
- Documentation of baseline versioning

**Week 2: Detection Script**
- [ ] Create `scripts/check-wasm-regressions.js` (200–250 lines)
  - Load baseline and current profiles
  - Compare each app per difficulty
  - Calculate regression percentages
  - Flag regressions per categories (NO ISSUE / INVESTIGATE / FIX REQUIRED)
  - Output formatted report to console
  - Write JSON log to `compliance/regression-alerts.json`
- [ ] Integrate into `pnpm wasm:build` postbuild hook
- [ ] Test with synthetic regression (modify baseline, verify detection)
- [ ] Document script usage and output format
...
### Sprint 2: Alerting & Dashboard (Weeks 3–4)
