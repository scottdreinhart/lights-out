 ...
 * Handles email, SMS text, and Slack notifications for regression alerts
 *
 ...
 *   SMS_EMAIL=4246664233@msg.fi.google.com  # SMS-to-email gateway (text on phone)
 *
...
export class NotificationService {
  constructor(config = {}) {
    ...
    this.smsEmail = process.env.SMS_EMAIL  // SMS-to-email gateway for urgent text alerts
  ...
  async sendRegressionAlert(regressions, severity, buildNumber, emailTemplate) {
    ...
    if (this.emailEnabled && this.mailer && emailTemplate) {
      promises.push(
        this.sendEmail(emailTemplate, buildNumber).catch((err) =>
    ...
    if (this.slackEnabled && this.slackWebhookUrl) {
      promises.push(
        this.sendSlackMessage(regressions, severity, buildNumber).catch((err) =>
  ...
  async sendEmail(emailTemplate, buildNumber) {
    ...
    try {
      ...
      console.log(`✅ Email sent to ${this.notificationEmail} (Build #${buildNumber})`)
      console.log(`   Message ID: ${info.messageId}`)

      // Send plain text SMS alert if SMS gateway configured
      // SMS gateways work best with plain text and short subjects
      if (this.smsEmail) {
        try {
          const mailOptions_sms = {
            from: this.smtpConfig.auth.user,
            to: this.smsEmail,
            subject: '[WASM ALERT]',  // Keep subject short for SMS
            text: emailTemplate.generateText(),  // Plain text for SMS gateway
            // No HTML for SMS gateways - they only handle text
          }
          await this.mailer.sendMail(mailOptions_sms)
          console.log(`📱 SMS text alert sent to ${this.smsEmail}`)
        } catch (smsError) {
          console.warn(`⚠️  SMS gateway send failed: ${smsError.message}`)
          // Don't throw - main email already sent
        }
      }
