...
function findTestFiles() {
  ...
  console.log('📋 Patterns to search:', allPatterns.length, 'patterns')
  allPatterns.forEach((p, i) => {
    try {
      glob.sync(p, { cwd: repoRoot })
    } catch (e) {
      console.error(`❌ Pattern ${i} failed: "${p}"`, e.message)
      throw e
    }
  })
  const testFiles = glob.sync(allPatterns, {
    cwd: repoRoot,
  })
  return testFiles.filter(f => !f.includes('node_modules') && !f.includes('dist') && !f.includes('release') && !f.includes('.next') && !f.includes('.turbo'))
}
