 ...
 * Phase 2 Week 4: Multi-Channel Notifications (Email + Slack + SMS)
 *
 * Detects performance regressions in WASM builds by comparing current profiles
 * against the established baseline and sends notifications via email, SMS, and/or Slack.
 *
 * Usage:
 ...
 *   SMS_EMAIL=4246664233@msg.fi.google.com      # SMS-to-email gateway (optional)
 ...
 *   - SMS text alert (if SMS_EMAIL configured)
...
import { RegressionEmailTemplate } from '../compliance/regression-email-template.js'
import { NotificationService } from '../compliance/regression-notification-service.js'
...
function logRegressions(results) {
  ...
  const buildNumber = alertsData.alerts.length + 1
  ...
  // Return build number for use in notifications
  results.buildNumber = buildNumber
}
...
async function sendNotifications(regressions, severity, results) {
  ...
  try {
    ...
    if (
      !isConfigured &&
      (process.env.EMAIL_ENABLED === 'true' || process.env.SLACK_ENABLED === 'true')
    ) {
      console.warn(
        `${COLORS.YELLOW}⚠️  Notifications configured but not properly set up${COLORS.RESET}`,
      )
      return
    }
