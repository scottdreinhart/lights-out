...
async function validateSchema(matrix) {
  ...
  if (!matrix.games || !Array.isArray(matrix.games)) {
    issues.push('Missing or invalid "games" array in matrix')
    console.log(`${symbols.fail} Invalid matrix structure\n`)
    return
  }

  const requiredFields = ['id', 'entries']
  let schemaValid = true

  matrix.games.forEach((game, idx) => {
    const missingFields = requiredFields.filter((field) => !game[field])
    if (missingFields.length > 0) {
      issues.push(`Game at index ${idx} missing fields: ${missingFields.join(', ')}`)
      schemaValid = false
    }

    if (game.entries && Array.isArray(game.entries)) {
      const entryFields = ['id', 'title', 'category', 'status']
      game.entries.forEach((entry, entryIdx) => {
        const missingEntryFields = entryFields.filter((field) => !entry[field])
        if (missingEntryFields.length > 0) {
          issues.push(
            `Game "${game.id}" entry ${entryIdx} missing: ${missingEntryFields.join(', ')}`,
          )
          schemaValid = false
        }
      })
  ...
  matrix.games.forEach((game) => {
    ...
    game.entries.forEach((entry) => {
      if (requiredGates.includes(entry.id) && entry.status === 'RED') {
        warnings.push(
          `Game "${game.id}" has critical quality gate RED: ${entry.id}`,
        )
      }
    })
  })
...
async function validateFeatureRequirements(matrix) {
  ...
  matrix.games.forEach((game) => {
    ...
    const featureEntries = game.entries.filter((e) =>
      featureCategories.includes(e.category),
    )
  ...
  sorted.forEach(([game, data]) => {
    ...
    console.log(
      `   • ${game.padEnd(25)} ${bar} ${data.percentage}% (${data.green}/${data.total})`,
    )
  })
  console.log()
}
...
function printSummary(matrix) {
  ...
  if (issues.length === 0) {
    ...
    console.log(`${colors.red}❌ Validation FAILED - ${issues.length} critical issue(s)${colors.reset}\n`)
  }
}
