...
function getSeverityColor(severity) {
  switch (severity) {
    case 'NO_ISSUE': return COLORS.GREEN;
    case 'INVESTIGATE': return COLORS.YELLOW;
    case 'FIX_REQUIRED': return COLORS.RED;
...
function getSeverityEmoji(severity) {
  switch (severity) {
    case 'NO_ISSUE': return '✅';
    case 'INVESTIGATE': return '⚠️ ';
    case 'FIX_REQUIRED': return '❌';
    default: return '  ';
  }
}
...
function detectRegressions(baseline, current) {
  ...
  baselineApps.forEach((appName) => {
    ...
    if (baselineMs === undefined || currentMs === undefined) {
      console.warn(`  ${COLORS.DIM}⚠️  ${appName}.${difficulty} missing baseline or current${COLORS.RESET}`);
      return;
    }
...
function printReport(results) {
  ...
  console.log(`  ${COLORS.YELLOW}⚠️  INVESTIGATE (5-10%):${COLORS.RESET}   ${summary.investigate} apps`);
  console.log(`  ${COLORS.RED}❌ FIX_REQUIRED (> 10%):${COLORS.RESET}   ${summary.fixRequired} apps`);
...
(function main() {
  ...
  console.log(`${COLORS.CYAN}📦 Loaded baseline v${baseline.version} from ${BASELINE_FILE}${COLORS.RESET}`);
  ...
  console.log(`${COLORS.CYAN}📝 Results logged to ${ALERTS_LOG_FILE}${COLORS.RESET}\n`);
  ...
  if (exitCode === 0) {
    ...
    console.log(`${COLORS.YELLOW}${COLORS.BOLD}⚠️  Minor regressions found - review recommended${COLORS.RESET}\n`);
  } else {
    console.log(`${COLORS.RED}${COLORS.BOLD}❌ Critical regressions found - fixup required${COLORS.RESET}\n`);
  }
