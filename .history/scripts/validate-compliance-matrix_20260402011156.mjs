...
async function validateConsistency(matrix) {
  ...
  if (!matrix.apps || typeof matrix.apps !== 'object') {
    console.log(`${symbols.fail} No apps data found`)
    console.log()
    return
  }
  ...
  Object.entries(statusCounts)
    ...
    .forEach(([status, count]) => {
      const percentage = ((count / appCount) * 100).toFixed(1)
      console.log(`   • ${status.padEnd(15)} ${count.toString().padStart(2)} (${percentage}%)`)
    })

  // Check summary accuracy
  if (matrix.summary && matrix.summary.total !== appCount) {
    warnings.push(`Summary total (${matrix.summary.total}) != actual app count (${appCount})`)
  }

  const passedCount = statusCounts['passed'] || 0
  const fixedCount = statusCounts['fixed'] || 0

  if (matrix.summary && matrix.summary.passed !== passedCount) {
    warnings.push(`Summary passed count (${matrix.summary.passed}) != actual (${passedCount})`)
  }

  if (matrix.summary && matrix.summary.fixed !== fixedCount) {
    warnings.push(`Summary fixed count (${matrix.summary.fixed}) != actual (${fixedCount})`)
  }
