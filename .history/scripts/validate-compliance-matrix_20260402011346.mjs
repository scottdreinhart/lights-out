...
function printSummary(matrix) {
  ...
  if (matrix.summary) {
    console.log(`Summary: ${matrix.summary.total} total, ${matrix.summary.passed} passed, ${matrix.summary.fixed} fixed`)
  }
  ...
  const status = issues.length === 0 ? `${colors.green}✅ PASS${colors.reset}` : `${colors.red}❌ FAIL${colors.reset}`
