...
async function main() {
  ...
  log(
    `Complete:    ${cellStatus.complete}/${expectedCells} (${((cellStatus.complete / expectedCells) * 100).toFixed(1)}%)`,
  )
  log(
    `Partial:     ${cellStatus.partial}/${expectedCells} (${((cellStatus.partial / expectedCells) * 100).toFixed(1)}%)`,
  )
  log(
    `Not Started: ${cellStatus['not-started']}/${expectedCells} (${((cellStatus['not-started'] / expectedCells) * 100).toFixed(1)}%)`,
  )
