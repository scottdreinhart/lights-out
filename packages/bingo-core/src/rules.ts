...
export class StandardBingoRules implements BingoRules {
  ...
  private checkFullHouse(card: BingoNumber[][], drawnNumbers: DrawnNumbers): boolean {
    return card.every((row) => row.every((num) => num === 0 || drawnNumbers.has(num)))
  }
  ...
  private checkFourCorners(
    card: BingoNumber[][],
    drawnNumbers: DrawnNumbers,
    rows: number,
    cols: number,
  ): boolean {
    const corners = [card[0][0], card[0][cols - 1], card[rows - 1][0], card[rows - 1][cols - 1]]
    return corners.every((num) => num === 0 || drawnNumbers.has(num))
  }

  private checkXPattern(
    card: BingoNumber[][],
    drawnNumbers: DrawnNumbers,
    rows: number,
    cols: number,
    ...
    if (rows !== cols) {
      return false
    } // X pattern requires square grid

    for (let i = 0; i < rows; i++) {
      // Check main diagonal
      const mainNum = card[i][i]
      if (mainNum !== 0 && !drawnNumbers.has(mainNum)) {
        return false
      }

      // Check anti-diagonal
      const antiNum = card[i][rows - 1 - i]
      if (antiNum !== 0 && !drawnNumbers.has(antiNum)) {
        return false
      }
    }
    return true
  ...
  private checkCenterCross(
    ...
    if (centerNum !== 0 && !drawnNumbers.has(centerNum)) {
      return false
    }
