...
interface SwipeConfig {
  ...
  maxDurationMs?: number // Ignore gesture if it takes longer than this duration
  onSwipeLeft?: () => void
  onSwipeRight?: () => void
  onSwipeUp?: () => void
  onSwipeDown?: () => void
...
/**
 * Detects swipe gestures on touch devices with velocity-based filtering.
...
export const useSwipeGesture = ({
  ...
  const handleTouchEnd = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (deltaTime <= 0 || deltaTime > maxDurationMs) {
        return
      }
    ...
    [maxDurationMs, threshold, velocityThreshold, onSwipeLeft, onSwipeRight, onSwipeUp, onSwipeDown],
  )
